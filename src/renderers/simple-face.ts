...
export const createEyelidPart: FacePartFactory<{
  ...
  ({ cx, cy, width, height, side }: { cx: number; cy: number; width: number; height: number; side: keyof FaceContext['eyes'] }) =>
  (_tick: number, path: CanvasPath, { eyes, emotion }: FaceContext) => {
    const eye = eyes[side]
    const w = width
...
export const createEyePart: FacePartFactory<{
  ...
  ({ cx, cy, radius = 8, side }: { cx: number; cy: number; radius?: number; side: keyof FaceContext['eyes'] }) =>
  (_tick: number, path: CanvasPath, { eyes }: FaceContext) => {
    const eye = eyes[side]
...
export const createMouthPart: FacePartFactory<{
  ...
  ({ cx, cy, minWidth = 50, maxWidth = 90, minHeight = 8, maxHeight = 58 }: { cx: number; cy: number; minWidth?: number; maxWidth?: number; minHeight?: number; maxHeight?: number }) =>
  (_tick: number, path: CanvasPath, { mouth }: FaceContext) => {
