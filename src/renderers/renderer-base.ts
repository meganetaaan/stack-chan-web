import { type Poco, PocoCanvasAdaptor } from "./poco";
import { Outline } from "./outline";
import type { CanvasPath } from "./outline";
...
export type FaceModifier<T = unknown> = ((
  tick: number,
  face: FaceContext,
  arg?: T
) => FaceContext) & {
...
export type FacePart<T = unknown> = (
  tick: number,
  path: CanvasPath,
...
export type FaceDecoratorFactory<T, V = unknown> = (
  param: T
) => FaceDecorator<V>;
...
export class RendererBase {
  ...
  constructor(option?: { canvas?: HTMLCanvasElement; poco?: Poco }) {
    if (option?.poco) {
      ...
      this._canvas =
        (option.poco as unknown as { getCanvas?: () => HTMLCanvasElement })
  ...
  update(
    interval = INTERVAL,
    faceContext: Readonly<FaceContext> = defaultFaceContext
  ): void {
    ...
    const shouldClear = !deepEqual(
      this.currentContext.theme,
      this.lastContext.theme
    );
    ...
    if (shouldRender) {
      ...
      [this.currentContext, this.lastContext] = [
        this.lastContext,
        this.currentContext,
      ];
    }
  ...
  clear(color: Color = [0x00, 0x00, 0x00]): void {
    ...
    this._poco.fillRectangle(
      this._poco.makeColor(...color),
      0,
      0,
      width,
      height
